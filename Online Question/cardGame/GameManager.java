...
class GameManager extends Thread {
	...
	public void run() {
		 ...
		 ** zero marks will be awarded if you modified the code or declared variable You
		 * have to write the code to perform the following tasks in sequential manner 1.
		 * This function runs till the round:Table is less than equal to maxRounds:Table. 2. Game
		...
		while (table.getRound() <= table.getmaxRounds()) {
			synchronized (table) {
				...
				System.out.println("\tRound " + table.getRound() + " 's Winner is: " + round_Winner.getPlayersName());
				table.getroundCards().clear();
				table.setroundCards(table.getroundCards());
				table.setRound(table.getRound() + 1);
				if (table.getRound() <= table.getmaxRounds()) {
					this.setPlayersTurn();
					table.setcurrentTurn(1);
					System.out.println("\tNew Round Begins! Round is " + table.getRound());
				}
				table.notifyAll();
			}
		}
		game_Winner = this.gameWinner();
		System.out.println("Winner is: " + game_Winner.getPlayersName());
