...
class Player extends Thread {
	...
	public void run() {
		...
		while (table.getRound() <= table.getmaxRounds()) {
			synchronized (table) {
				while (table.getcurrentTurn() != this.turn && table.getRound() <= table.getmaxRounds()) {
					try {
						table.wait();
				...
				if(table.getRound()<=table.getmaxRounds()) {
					...
					cardToThrow=this.selectCardToThrow();
					System.out.println("Player "+this.getPlayersName()+" selected "+cardToThrow.getcategory()+cardToThrow.getnumber());
					table.getroundCards().add(cardToThrow);
					table.setroundCards(table.getroundCards());
					this.getplayerCards().remove(cardToThrow);
