...
class Answer implements Comparable<Answer> {
	...
	public String toString() {
		...
		String s2 = "Answered Sequence : [ " + answerSequence[0] + "  " + answerSequence[1] + "  " + answerSequence[2]
				+ "  " + answerSequence[3] + " ] " + "\t";
...
class AnswerBoard {
	...
	public void lockAnswerBoard() {
		isLocked = true;
		System.out.println("AnswerBoard Locked");
	}

	public void unlockAnswerBoard() {
		isLocked = false;
		System.out.println("AnswerBoard UnLocked");
	}
	...
	public synchronized boolean addAnswer(Answer ans, int place) {
		if (!isLocked) {
			...
			answerList.set(place, ans);
			return true;
		} else {
			System.out.println("Answer Refused");
			return false;
		}
	}

	public void initializeAnswerList() {
		answerList.set(0, null);
		answerList.set(1, null);
		answerList.set(2, null);
		answerList.set(3, null);
		answerList.set(4, null);
		answerList.set(5, null);
		answerList.set(6, null);
		answerList.set(7, null);
		answerList.set(8, null);
		answerList.set(9, null);
...
class TimerThread extends Thread {
	...
	public void setTimeOver(boolean v) {
		timeOver = v;
	}

	public synchronized boolean getTimeOver() {
		return timeOver;
	}
	...
	public void run() {
		try {
			while (true) {
				synchronized (this) {
					while (suspendFlag) {
						System.out.println("Timer has been Suspended");
						wait();
					}
					for (int k = 10; k >= 0; k--) {
...
class MasterController extends Thread {
	...
	MasterController(ArrayList<Player> playerList, AnswerBoard answerBoard, JTextField timerTextField,
			JTextArea resultTextArea, JTextField oT1, JTextField oT2, JTextField oT3, JTextField oT4, JTextField ct1,
			JTextField ct2, JTextField ct3, JTextField ct4, JTextField t5, JTextField t6, JButton b) {
				...
				for (int k = 0; k < 10; k++) {
					...
					for (int i = 0; i < 10; i++) {
						if (answerList.get(i) == null) {
							playerList.get(i).chgbtncolor(5);
							continue;
						}
						Answer ans = answerList.get(i);
						int[] x1 = ans.getAnswerSequence();
						if (seq[0] == x1[0] && seq[1] == x1[1] && seq[2] == x1[2] && seq[3] == x1[3])
							playerList.get(i).chgbtncolor(4);
						else
							playerList.get(i).chgbtncolor(3);
					}
...
//	Master Control Thread
	...
	public static void populateQuestionList() {
		...
		questionList.add(new Question(Q1, chQ1, crSeq1));
		questionList.add(new Question(Q2, chQ2, crSeq2));
		questionList.add(new Question(Q3, chQ3, crSeq3));
		questionList.add(new Question(Q4, chQ4, crSeq4));
		questionList.add(new Question(Q5, chQ5, crSeq5));
		questionList.add(new Question(Q6, chQ6, crSeq6));
		questionList.add(new Question(Q7, chQ7, crSeq7));
		questionList.add(new Question(Q8, chQ8, crSeq8));
		questionList.add(new Question(Q9, chQ9, crSeq9));
	...
	public static void createThreads() {
		...
		playerList.add(fffP1);
		playerList.add(fffP2);
		playerList.add(fffP3);
		playerList.add(fffP4);
		playerList.add(fffP5);
		playerList.add(fffP6);
		playerList.add(fffP7);
		playerList.add(fffP8);
		playerList.add(fffP9);
		playerList.add(fffP10);
		master = new MasterController(playerList, answerBoard, timerTextField, resultDisplayArea, T1, T2, T3, T4, CT1,
				CT2, CT3, CT4, winnerText, timetakenText, resetB);
	}
	...
	public static void displayFrame() {
		...
		playerPanel.add(playerbtn1);
		playerPanel.add(playerbtn2);
		playerPanel.add(playerbtn3);
		playerPanel.add(playerbtn4);
		playerPanel.add(playerbtn5);
		playerPanel.add(playerbtn6);
		playerPanel.add(playerbtn7);
		playerPanel.add(playerbtn8);
		playerPanel.add(playerbtn9);
		playerPanel.add(playerbtn10);
		...
		p2.add(timerLabel);
		p2.add(timerTextField);
		p3.add(questionLabel);
		p3.add(questionArea);
		...
		p10.add(winner);
		p10.add(winnerPlayer);
		p10.add(winnerText);
		p10.add(timeTaken);
		p10.add(timetakenText);
		gamePanel.add(gameLabel);
		gamePanel.add(gamenoText);
